...
namespace Unity.IO.Compression {
    ...
    public class DeflateStream : Stream {
        ...
        public override int Read(byte[] array, int offset, int count) {
			...
			UnityEngine.Debug.Log("DeflateStream.Read(...) internal stream length is " + _stream.Length);


			// read from stream into buffer
			int bytes = _stream.Read(buffer, 0, buffer.Length);
			UnityEngine.Debug.Log("_stream type " + _stream.GetType() + " length " + _stream.Length + " buffer bytes " + bytes);
			// pass buffer to inflater
			inflater.SetInput(buffer, 0, bytes);
			UnityEngine.Debug.Log("inflater available output " + inflater.AvailableOutput);
			
			
			while (true) {
				UnityEngine.Debug.Log("DeflateStream.Read(...) iteration " + (++iter));

				// inflate into array
				bytesRead = inflater.Inflate(array, currentOffset, remainingCount);
				//UnityEngine.Debug.Log("read " + bytesRead + " bytes...");
				...
				if ( bytes == 0) {
					UnityEngine.Debug.LogWarning("bytes == 0 in DeflateStream.Read(...) ... End of stream?");
                    break;
                }
